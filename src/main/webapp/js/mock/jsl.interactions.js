    ...
    function init() {
        ...
        $('#json-btn').click(function () {
            ...
            if (jsonVal.substring(0, 4).toLowerCase() === "http") {
                $.post("/ajax/goto_url_get_json", {"url": jsonVal}, function (responseObj) {
					if(responseObj.error == true)
					{
						$('#results').text(responseObj.result).addClass('error');;
					}
					else
					{
						$('#json_input').val(JSON.stringify(responseObj.content));
						validate();
					}
                }, 'json');
            } else {
