			...
			<form id="addForm" action="" method="post">
				...
				<div class="form-group">
					<label for="nf-email">角色名称</label> <input type="input"
						name="roleName" class="form-control" placeholder="角色名称" value="${role.roleName}">
				</div>

				<div class="form-group">
					<label for="nf-password">使用状态</label> <label
						class="switch switch-success"> <input name="useStatus"
						type="checkbox" class="switch-input" <c:if test="${role.status==1}">checked=""</c:if>> <span
						...
						<div class="">
							...
							<c:forEach var="r" items="${map.value}" varStatus="stat">
								<label class="checkbox-inline" for="inline-checkbox1"> <input
									...
									value="on" 
									<c:if test="${not empty rightMap[r.id]}">checked=""</c:if>
									 > ${r.rightName}
							</c:forEach>
						</div>
					</c:forEach>
				...
				<div class="form-group">
					<input class="select-all" type="checkbox" name="ddad" />全选  
				</div>
		</div>
...
<script type="text/javascript" src="/manager/js/role/toModify.js"></script>
