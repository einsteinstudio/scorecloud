		...
		<div class="panel-body">
			<form id="addForm" action="" method="post">
				<div class="form-group">
					...
					<select  id="parentId" name="parentId" class="form-control" size="1">
						<option value="" >无</option>
				    	 <c:forEach var="vo" items="${selectVos}">  
				    	 	<option value="${vo.value}" <c:if test="${vo.value==pId}">selected="selected"</c:if> >${vo.name}</option>
				    	 </c:forEach>                   
				     </select>
				</div>
				<div class="form-group">
					<label for="nf-email">部门名称</label> <input type="input" name="name"
						class="form-control" placeholder="部门名称" value="">
				</div>
				<div class="form-group">
					<label for="nf-email">排序号</label> <input type="input" name="weight"
						class="form-control" placeholder="排序号">
				</div>
				<div class="form-group">
				...
				<div class="form-group">
					<label for="nf-email">部门职责</label> <input type="input" name="duty"
						class="form-control" placeholder="部门职责">
				</div>
		</div>
			...
			$(form).ajaxSubmit({
				...
				success : function(data) {
					if (data.result == "success") {
						window.location.href = "list.htm?pId="+$("#parentId").val();
					} else {
						alert(data.msg);
					}
				}
